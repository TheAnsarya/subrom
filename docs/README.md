# Subrom Documentation

Welcome to the Subrom documentation. Subrom is a ROM management and verification toolkit for retro gaming enthusiasts.

## What is Subrom?

Subrom helps you:

- **Verify ROMs** against DAT files from No-Intro, TOSEC, MAME, and more
- **Organize** your ROM collection with intelligent renaming and folder structures
- **Track** ROMs across multiple drives, including offline storage
- **Scan** archives (ZIP, 7z, RAR) without extraction
- **Identify** duplicates, bad dumps, and missing ROMs

## Quick Start

1. **Install Subrom** - Download and run the installer
2. **Add DAT Files** - Import DAT files from your preferred provider
3. **Scan Your ROMs** - Point Subrom at your ROM folders
4. **Verify** - See which ROMs match, are missing, or are duplicates

## Documentation Index

### Getting Started

- [Installation](getting-started/installation.md)
- [First Scan](getting-started/first-scan.md)
- [Understanding Results](getting-started/understanding-results.md)

### Features

- [DAT File Management](features/dat-management.md)
- [ROM Scanning](features/scanning.md)
- [Archive Support](features/archives.md)
- [Drive Management](features/drives.md)

### Reference

- [Supported DAT Formats](reference/dat-formats.md)
- [Supported Archive Formats](reference/archive-formats.md)
- [Configuration Options](reference/configuration.md)
- [API Reference](reference/api.md)

### Guides

- [Organizing with 1G1R](guides/1g1r.md)
- [Managing Multiple Drives](guides/multiple-drives.md)
- [Offline Storage](guides/offline-storage.md)

## Support

- [FAQ](faq.md)
- [Troubleshooting](troubleshooting.md)
- [GitHub Issues](https://github.com/TheAnsarya/subrom/issues)

## License

Subrom is open source software licensed under the MIT License.
